<script>
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const url = `${PUBLIC_API_BASE_URL}/api/artworks/${encodeURIComponent(artworkId)}${forceParam}`;
...
<div class="modal-backdrop" on:click={onCancel} role="button" tabindex="-1">
  <div class="modal-content" on:click|stopPropagation role="dialog" aria-modal="true">
    ...
    {:else if step === 'confirm-soft'}
      <h2>Soft Delete Artwork?</h2>
      <div class="confirmation-message">
        <p>The artwork will be hidden and marked for deletion.</p>
        <p><strong>You can restore it within 30 days</strong> from your My Artworks page.</p>
        <p class="warning">After 30 days, it will be automatically deleted permanently.</p>
      </div>
    ...
    {:else if step === 'confirm-hard'}
      <h2 class="danger-header">PERMANENTLY DELETE ARTWORK?</h2>
      <div class="confirmation-message danger">
        <p class="danger-text"><strong>This action CANNOT be undone!</strong></p>
        <p>The artwork and all associated photos will be permanently deleted.</p>
